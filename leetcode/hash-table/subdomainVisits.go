...
func subdomainVisits(cpdomains []string) []string {
	...
	for _, paired := range cpdomains {
		countPaired := strings.Split(paired, " ")
		count, _ := strconv.Atoi(countPaired[0])
		domain := strings.Split(countPaired[1], ".")
		for i := range domain {
			maps[strings.Join(domain[i:], ".")] += count
		}
	}

	res, idx := make([]string, len(maps)), 0
	for domain, count := range maps {
		res[idx] = fmt.Sprintf("%d %s", count, domain)
		idx++
	}
	return res
}
